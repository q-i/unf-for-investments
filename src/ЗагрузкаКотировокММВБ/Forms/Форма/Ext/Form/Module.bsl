...
Процедура СохранитьДанныеМосбиржиЗаДатуВФайл(ЗаписьТекста, ИдРежимаТоргов, ТекДата)	
	...
	Сервер = "iss.moex.com";
	АдресСтраницы = "/iss/history/engines/stock/markets/shares/boards/[ИдРежимаТоргов]/securities.xml?date=[ГГГГ]-[ММ]-[ДД]";
	АдресСтраницы = АдресСтраницы + "&iss.meta=off"; // отключаем вывод мета-информации
	АдресСтраницы = АдресСтраницы + "&history.columns=TRADEDATE,SECID,LEGALCLOSEPRICE"; // выводим только нужные нам колонки
	...
	Пока ЕстьДанные Цикл
		...
		XML_document = Фабрика.ПрочитатьXML(ЧтениеXML);
		Для Каждого XML_data Из XML_document.data Цикл
			Если XML_data.id = "history" Тогда
				XML_rows = XML_data.rows;
				Если XML_rows.Свойства().Количество() > 0 Тогда
					Для Каждого XML_row Из XML_rows.row Цикл
						Стр =   ""
								+ XML_row.TRADEDATE + ","
								+ XML_row.SECID + ","
								+ XML_row.LEGALCLOSEPRICE;
